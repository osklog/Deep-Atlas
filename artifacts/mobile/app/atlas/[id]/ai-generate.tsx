...
export default function AiGenerateScreen() {
  ...
  async function generate() {
    ...
    const body = {
      ...
      nodes: atlas.nodes.map((n) => ({ type: n.type, title: n.title, note: n.note, tags: n.tags })),
  ...
  return (
    <View style={[styles.container, { paddingTop: insets.top + 4 }]}>
      <View style={styles.header}>
        <Pressable onPress={() => { abortRef.current?.abort(); router.dismiss(); }} style={styles.closeBtn}>
        ...
        {output && !loading ? (
          <Pressable onPress={copyToClipboard} style={styles.copyBtn}>
            <Feather name={copied ? "check" : "copy"} size={16} color={copied ? "#6EF08A" : C.tint} />
          </Pressable>
        ) : (
          <View style={{ width: 36 }} />
        )}
      ...
      {atlas && (
        <View style={styles.atlasInfo}>
          <Text style={styles.atlasInfoText} numberOfLines={1}>{atlas.title}</Text>
      ...
      <View style={styles.modeGrid}>
        {MODES.map((m) => (
          ...
          >
            ...
            />
            <Text style={[styles.modeLabel, selectedMode === m.key && styles.modeLabelSelected]}>
              {m.label}
            </Text>
...
const styles = StyleSheet.create({
  ...
  atlasInfo: {
    ...
    backgroundColor: C.backgroundCard, borderRadius: 10, padding: 10,
    borderWidth: 1, borderColor: C.borderSubtle,
  },
  atlasInfoText: { fontSize: 14, fontFamily: "Inter_600SemiBold", color: C.text },
  atlasInfoStat: { fontSize: 12, color: C.textMuted, fontFamily: "Inter_400Regular", marginTop: 2 },
  modeGrid: {
    flexDirection: "row", flexWrap: "wrap",
    paddingHorizontal: 12, gap: 8, marginBottom: 12,
  },
  modeCard: {
    width: "47%", backgroundColor: C.backgroundCard,
    borderRadius: 12, borderWidth: 1, borderColor: C.borderSubtle,
    padding: 12, gap: 4,
  },
  modeCardSelected: { borderColor: C.tint + "60", backgroundColor: C.tintGlow },
  modeLabel: { fontSize: 13, fontFamily: "Inter_600SemiBold", color: C.textSecondary, marginTop: 2 },
  modeLabelSelected: { color: C.tint },
  modeDesc: { fontSize: 11, fontFamily: "Inter_400Regular", color: C.textMuted, lineHeight: 16 },
  generateBtn: {
    flexDirection: "row", alignItems: "center", justifyContent: "center",
    gap: 8, marginHorizontal: 16, paddingVertical: 14,
    borderRadius: 12, backgroundColor: C.tint, marginBottom: 12,
  },
  generateBtnLoading: { opacity: 0.7 },
  generateBtnText: { fontSize: 16, fontFamily: "Inter_600SemiBold", color: C.background },
  outputScroll: { flex: 1 },
  outputContent: { paddingHorizontal: 16, paddingTop: 4 },
  outputText: {
    fontSize: 15, color: C.text, fontFamily: "Inter_400Regular", lineHeight: 26,
    backgroundColor: C.backgroundCard, borderRadius: 12, padding: 16,
    borderWidth: 1, borderColor: C.borderSubtle,
  },
  outputLoading: { alignItems: "center", gap: 10, paddingTop: 20 },
  outputLoadingText: { color: C.textMuted, fontFamily: "Inter_400Regular", fontSize: 14 },
  outputPlaceholder: {
